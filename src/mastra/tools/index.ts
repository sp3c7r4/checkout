...
export const readAllStoreProducts = createTool({
  ...
  execute: async ({ context: { user_id } }) => {
    try {
      ...
      CheckoutEmitter.emit("sendStoreProducts", { data: mutated_resp, user_id, message });
...
export const placeOrderForCartItemsTool = createTool({
  ...
  execute: async ({ context: { user_id }}) => {
    try {
      ...
      const metadata = { user_id: user_id, business_id: current_business_id, product: cart.products };
